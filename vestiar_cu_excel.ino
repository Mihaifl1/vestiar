/* esp_vestiar_complete_dual_tz.ino
   ...
   - Wiegand (keypad + RFID)
   - Relay control (puls / hold + autolock 3s pt card/parolă)
   ...
   - Web UI (programări + carduri + fus orar) cu CORS
   - Status MQTT include: ip, mac, rssi, door, pwr
   - Publică și topicul info: locker/<ID>/info (ip, freeHeap)
   - UI citește din PHP (vestiar_dbinfo.php) ultimul card + numărători
*/
...
const char* PHP_CARDS_ADD    = "http://192.168.66.240/vestiar_cards_add.php";  // adăugare card
const char* PHP_CARDS_REMOVE = "http://192.168.66.240/vestiar_cards_remove.php";// ștergere card
...
void setRelayState(bool on, bool byCard = false, bool autoLock = false) {
  ...
  // eveniment LOCK/UNLOCK către MQTT + DB
...
void publishStatus() {
  ...
  httpPostJson(PHP_STATUS_URL, out); // trimitem status și în DB
}
void publishInfo() {
...
void processWiegand() {
  ...
  if (treatAsCard) {
    ...
    if (enrollMode) {
      ...
      String out; serializeJson(d, out);
      // în locul MQTT add imediat — trimitem și în DB:
...
function makeRuleElement(r, idx){
  ...
  note.type='text'; 
  note.value=r.note||''; 
  note.placeholder='notă (opțional)'; 
...
document.getElementById('btnExportCSV').onclick = () => {
  let csv = "Nr,Zile,Ora,Acțiune,Notă\n";
  rules.forEach((r, i) => {
...
// ---------- WEB API helpers ----------
// ---------- handleRoot: servește pagina local din LittleFS ----------

void handleRoot() {
  Serial.println("[HTTP] Servesc interfața din LittleFS...");
  ...
  if (LittleFS.exists("/index.html")) {
    ...
    Serial.println("⚠️ Fallback: folosesc versiunea încorporată (PROGMEM)");
  }

  // Înlocuiește variabila din HTML cu IP-ul tău PHP
...
// ---------- Trimite evenimente CARD la serverul PHP (păstrat pentru compat) ----------
void sendEventToServer(unsigned long card24, unsigned int facility, unsigned int cardNumber, bool ok) {
  ...
  if (T_EVENT.length()) mqtt.publish(T_EVENT.c_str(), json.c_str(), false); // și MQTT
}
...
void setup() {
  ...
  loadTZ();                 // încarcă TZ
  loadCardsFromFS();        // încarcă carduri
  loadCode();               // încarcă parola tastaturii (Keypad Master Code)
  ...
  configTime(tz_offset, DST_OFFSET, NTP1, NTP2);
// După ce te-ai conectat la WiFi
Serial.printf("\nWiFi OK, IP=%s\n", WiFi.localIP().toString().c_str());
...
Serial.println("OTA ready, update available via Wi-Fi!");
  ...
  registerCodeApi();  // înregistrează rutele pentru /api/code
  loadCode();         // încarcă codul master din LittleFS
...
void loop() {
  ...
  // auto-lock după 3s pentru card/parolă
  ...
  // re-trimite info la schimbări IP/RSSI/freeHeap
